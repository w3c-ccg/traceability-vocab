...
it('should generate useful periodic-table-data.json from ontologies', async () => {
  ...
  classes.forEach((classData) => {
    if (classData['chebi:formula']) {
      ...
      const alternateIds = classData['oboInOwl:hasAlternativeId']
        ? classData['oboInOwl:hasAlternativeId'].map((d) => d._)
        : [];

      const dbXrefs = classData['oboInOwl:hasDbXref']
        ? classData['oboInOwl:hasDbXref'].map((d) => d._)
        : [];

      const about = classData.$['rdf:about'];

      const ptJson = Object.values(pt).find((el) => el.symbol === formula);

      const mergedIds = [...alternateIds, ...dbXrefs].map((d) => {
